...
const FilterSection = () => {
  ...
  return (
    <aside className="hidden lg:block w-64 p-4 rounded-lg  lg:mr-6">
      <h4 className="text-2xl font-bold mb-6 text-gray-500">Filter by:</h4>
      ...
      <div className="mb-6">
        ...
        >
          <h4 className="font-semibold text-lg text-gray-500">Promotions</h4>
          <span className="text-sm text-gray-500 transition-transform duration-300">{isPromotionOpen ? '▲' : '▼'}</span>
        </div>
        <div
        ...
        >
          <label className="flex items-center cursor-pointer mt-3">
            <input type="checkbox" className="hidden peer" onChange={(e) => setShowPromotions(e.target.checked)} />
            <div className="w-5 h-5 border-2 border-gray-300 rounded-full flex items-center justify-center peer-checked:bg-teal-500 peer-checked:border-teal-500 transition">
              <svg
                ...
                className="h-4 w-4 text-black hidden peer-checked:block"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
            ...
            <span className="ml-3 text-gray-600 text-sm hover:border-teal-500">Only show promotions</span>
          </label>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          <h4 className="font-semibold text-lg text-gray-500">Category</h4>
          <span className="text-sm text-gray-500 transition-transform duration-300">{isCategoryOpen ? '▲' : '▼'}</span>
        </div>
        <div
        ...
        >
          <ul className="space-y-3 mt-2">
            {['All', 'Make', 'Skin', 'Hair', 'Bath & Body', 'Fragrance', 'Health & Wellness'].map((category) => (
              <li key={category}>
                <button className="text-gray-600 hover:text-teal-500">{category}</button>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div>
        ...
        >
          <h4 className="font-semibold text-lg text-gray-500">Review score</h4>
        ...
        >
          <ul className="space-y-2 text-sm mt-2">
            {[
              { stars: '★★★★★', color: 'text-yellow-500', value: 5 },
              { stars: '★★★★☆', color: 'text-yellow-400', value: 4 },
              { stars: '★★★☆☆', color: 'text-yellow-300', value: 3 },
              { stars: '★★☆☆☆', color: 'text-yellow-200', value: 2 },
              { stars: '★☆☆☆☆', color: 'text-yellow-100', value: 1 },
            ].map((rating, idx) => (
              <li key={idx} className="flex items-center space-x-2">
                ...
                />
                <label htmlFor={`rating-${rating.value}`} className="cursor-pointer flex items-center space-x-2">
                  {/* ปุ่มวงกลม */}
                  <span
              ...
              hover:bg-teal-500 hover:border-teal-500 transition"
                  ></span>
                  {/* ดาวสีเหลือง */}
                  <span className="text-yellow-500 text-2xl leading-none">{rating.stars}</span>
                </label>
              </li>
            ))}
          </ul>
